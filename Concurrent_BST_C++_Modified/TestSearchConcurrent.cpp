...
void run(Node* root)
{
    ...
    for (int i = num_iterations; num_iterations < MAX_ITERATION1; num_iterations++)
    {
        ...
        //cout << search(root, rand_var) << endl;
        search(root, rand_var);
    }
...
{
    for (int i = build_tree_iterations; build_tree_iterations < 100000; build_tree_iterations++)
    {
        int rand_var = 1 + (rand() % 90000);
