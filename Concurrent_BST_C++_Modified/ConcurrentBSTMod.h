...
Node* getSecondToTop(stack<Node*> stack)
{
    Node* temp = stack.top();
    stack.pop();
    Node* res = stack.top();
...
SeekRecord seek(Node* root, int key, stack<Node*> stack)
{
...
SeekRecord seek_og(Node* root, int key)
{
    Node* ancestor = root;
...
{
    SeekRecord seekRecord = seek_og(root, key);
...
bool remove(Node* root, int key)
{
    ...
    while (true)
    {
        SeekRecord seekRecord = seek(root, key, stack);
        Node* parent = seekRecord.parent;
...
bool insert(Node* root, int key)
{
    ...
    while (true)
    {
        ...
        if ((GET_ADDR(seekRecord.terminal)->key) != key)
        {
            ...
            newSibling->key = GET_ADDR(terminal)->key;
